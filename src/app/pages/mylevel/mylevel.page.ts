...
export class LevelPage implements OnInit {
    ...
    this.objUserService.getuserdetails(loguserdata).subscribe(
      (data:any) => {
            ...
            if(data.status=="success"){  
              ...
              this.arrOurclasses = Object.entries(data.user_level);
            }else{
  ...
  fnOnchangeinput($event){
    if($event.target.id == "level"){     
      this.userdata.ulevel = $event.target.value
    }
  ...
  onSave() {
   this.loadctrlPresent();
    if(this.userdata.ulevel){ 
      ...
      this.objUserService.updateuserlevelreqdata(this.userdata).subscribe(
        (data:any) => {
