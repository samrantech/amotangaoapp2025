...
@Component({
  ...
  templateUrl: './tangotripsitems.page.html',
  styleUrls: ['./tangotripsitems.page.scss'],
})
export class TangotripsitemsPage implements OnInit {
  ...
  fnGetEventLists() {
    if (this.eventId) {
      ...
      this.objCourserv.getTangotripsItemsById(postFdata).subscribe(
        (data: any) => {         
          if (data.status == 'success') {
            ...
            if(pageData){
              this.cpagedata.page_title = pageData?.trip_title + ", " +  pageData?.short_description;
