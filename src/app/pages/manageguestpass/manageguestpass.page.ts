...
@Component({
  ...
  templateUrl: './manageguestpass.page.html',
  styleUrls: ['./manageguestpass.page.scss'],
})
export class LevelPage implements OnInit {
  ...
  async presentToast(position,msg) {
    ...
    this.objUserService.getuserdetails(loguserdata).subscribe(
      (data:any) => {
            ...
            if(data.status=="success"){  
              ...
              this.userdata.ulevel = uddata.user_level + "";
              this.userdata.uuid = uddata.user_id; 
              
              this.arrOurclasses = data.return_days;
            }else{
  ...
  fnOnchangeinput($event){
    if($event.target.id == "guest_name"){     
      this.userdata.guest_name = $event.target.value
    }
    if($event.target.id == "guest_email"){     
      this.userdata.guest_email = $event.target.value
    }
    if($event.target.id == "date"){     
      this.userdata.date = $event.target.value
    }
  ...
  onSave() {
   this.loadctrlPresent();
    //if(this.userdata.ulevel){ 
      ...
      this.objUserService.fncreateguestpass(this.userdata).subscribe(
        (data:any) => {
