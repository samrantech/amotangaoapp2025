...
@Component({
  ...
  templateUrl: './managekeymapping.page.html',
  styleUrls: ['./managekeymapping.page.scss'],
})
...
fngetuserpartnersearch(page: number = 1) {
  ...
  this.objUserService.fngetkeyconcepts(postFdata).subscribe(
    (data:any) => {           
          if(data.status=="success"){                
            ...
            this.arrkeyoptions = data.keyconcepts_pagination.data;
        this.pagination = data.keyconcepts_pagination;
    this.currentPage = data.keyconcepts_pagination.current_page;
...
fnOnchangefilterlevel($event){
  ...
  if($event.target.id == "filterlevel"){     
...
onEdit(item){
  ...
  this.userdata.etitle = item.name;
  this.userdata.short_description = item.description;
  this.userdata.media_source = item.media_source;
  this.userdata.level = item.level;
...
onSave() {
  ...
  if(this.fnValidateform()==true){ 
    this.objUserService.fncreupkeyconcepts(loguserdata).subscribe(
      (data:any) => {
            if(data.status=="success"){  
              this.fngetuserpartnersearch();
              this.part_section = 0;
...
onremove(item){
  let loguserdata = {
    ...
    this.objUserService.fndeletkeyconceptsbyid(loguserdata).subscribe(
      (data:any) => {
            if(data.status=="success"){  
...
onView(item){
  ...
  this.router.navigateByUrl('/managekeyoptionmapping', navextra);
}
create_new(){
  this.rehis = 1;
}
