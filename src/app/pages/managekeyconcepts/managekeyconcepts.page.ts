...
@Component({
  ...
  templateUrl: './managekeyconcepts.page.html',
  styleUrls: ['./managekeyconcepts.page.scss'],
})
...
handleInput(event: Event) {
  ...
  this.objUserService.getallusers(postFdata).subscribe(
    (data:any) => {
          this.dismissLoadctrl();            
          if(data.status=="success"){   
            this.arrUserlists = data.userslist;
          }
...
onSave() {
  ...
  if(this.fnValidateform()==true){ 
    this.objUserService.fnuserkeyconceptmapping(loguserdata).subscribe(
      (data:any) => {
            if(data.status=="success"){  
              this.fngetuserpartnerdetails(this.user_email);
              this.part_section = 0;
              this.crud_section = 0;
              this.resubs = 1;
              this.presentToast('top','Updated Successfully!');
...
onView(item){
  ...
  this.fngetuserpartnerdetails(item.email);
              // this.resubs = 0;
  this.part_section = 1;
              // this.viewconcept = 1;
