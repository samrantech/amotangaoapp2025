...
@Component({
  ...
  templateUrl: './mevents.page.html',
  styleUrls: ['./mevents.page.scss'],
})
export class MeventsPage implements OnInit {
  public islogged: any;
  data$: Observable<any>;
  data: any;
        ...
        if (result == true) {
          ...
          this.subParams = this.router.params.subscribe((params) => {
            if (this.route.getCurrentNavigation().extras.state != undefined) {
              this.eventId =
                this.route.getCurrentNavigation().extras.state.eventid;
        ...
        (data: any) => {
          ...
          if (data.status == 'success') {
            ...
            if(pageData){
              ...
              this.cpagedata.page_content = pageData.edescription;
  ...
  async goToModel(itemDetails) { 
    const modal = await this.modalController.create({
      ...
      componentProps: { itemData: itemDetails,itemCategory: "eventbooking" }
    });
    await modal.present();
  }
